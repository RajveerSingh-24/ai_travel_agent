        ...
        <div className="flex flex-1 min-h-0 overflow-hidden">
          ...
          <aside className={`hidden lg:flex flex-col shrink-0 border-border bg-card transition-all duration-300 ${isSidebarOpen ? "w-64 border-r" : "w-0 overflow-hidden border-r-0"}`}>
            <div className="p-4 shrink-0 w-64">
              ...
              <button
                type="button"
                onClick={() => setIsBookingHistoryOpen(!isBookingHistoryOpen)}
                className={`w-full flex items-center gap-1.5 rounded-lg px-3.5 py-2 text-xs font-semibold transition-all duration-200 shadow-sm border mt-2 ${isBookingHistoryOpen ? "bg-emerald-500/10 border-emerald-500/20 text-emerald-600 dark:text-emerald-400" : "bg-background border-border text-foreground hover:bg-muted"}`}
              >
